    ...
    public class IntegrationWatcher : IDisposable
    {
        ...
        private void LaunchIntegration(CustomIntegration integration)
        {
            ...
            try
            {
                ...
                if (process != null)
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Integration '{integration.Name}' launched for game ID '{integration.GameID}' (PID {process.Id}).");
                    _activeIntegrations[process.Id] = integration;
                }
            }
            catch (Exception ex)
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Failed to launch integration '{integration.Name}': {ex.Message}");
            }
        }
        ...
        private void TerminateProcess(int pid)
        {
            ...
            try
            {
                ...
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Terminated integration process (PID {pid}).");
            }
            catch (Exception)
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Failed to terminate process (PID {pid}), likely already exited.");
            }
        }
