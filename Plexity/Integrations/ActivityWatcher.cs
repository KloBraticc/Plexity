...
using Plexity.Models.PlexityRPC;
    ...
    public class ActivityWatcher : IDisposable
    {
        ...
        public async void Start()
        {
            ...
            if (String.IsNullOrEmpty(LogLocation))
            {
                ...
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Opening Roblox log file...");
                ...
                while (true)
                {
                    ...
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Could not find recent enough log file, waiting... (newest is {logFileInfo.Name})");
            ...
            App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Opened {LogLocation}");
        ...
        private void ReadLogEntry(string entry)
        {
            ...
            if (_logEntriesRead <= 1000 && _logEntriesRead % 50 == 0)
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Read {_logEntriesRead} log entries");
            else if (_logEntriesRead % 100 == 0)
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Read {_logEntriesRead} log entries");

            if (entry.Contains(GameLeavingEntry))
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "User is back into the desktop app");
                
                OnAppClose?.Invoke(this, EventArgs.Empty);

                if (Data.PlaceId != 0 && !InGame)
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "User appears to be leaving from a cancelled/errored join");
                    Data = new();
                }
            }
            ...
            if (!InGame && Data.PlaceId == 0)
            {
                ...
                if (entry.Contains(GameJoiningPrivateServerEntry))
                {
                    ...
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to assert format for game join private server entry");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }
                ...
                else if (entry.Contains(GameJoiningEntry))
                {
                    ...
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Failed to assert format for game join entry");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }
                    ...
                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Joining Game ({Data})");
                }
            }
            else if (!InGame && Data.PlaceId != 0)
            {
                ...
                if (entry.Contains(GameJoiningUniverseEntry))
                {
                    ...
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to assert format for game join universe entry");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }
                ...
                else if (entry.Contains(GameJoiningUDMUXEntry))
                {
                    ...
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to assert format for game join UDMUX entry");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }
                    ...
                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Server is UDMUX protected ({Data})");
                }
                else if (entry.Contains(GameJoinedEntry))
                {
                    ...
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Failed to assert format for game joined entry");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }

                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Joined Game ({Data})");
            ...
            else if (InGame && Data.PlaceId != 0)
            {
                ...
                {
                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Disconnected from Game ({Data})");
                ...
                {
                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Initiating teleport to server ({Data})");
                ...
                else if (entry.Contains(GameMessageEntry))
                {
                    ...
                    if (match.Groups.Count != 2)
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Failed to assert format for RPC message entry");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }
                    ...
                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Received message: '{messagePlain}'");

                    if ((DateTime.Now - LastRPCRequest).TotalSeconds <= 1)
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Dropping message as ratelimit has been hit");
                        return;
                    }
                    ...
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to parse message! (JSON deserialization threw an exception)");
                        return;
                    }

                    if (message is null)
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to parse message! (JSON deserialization returned null)");
                        return;
                    }

                    if (string.IsNullOrEmpty(message.Command))
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to parse message! (Command is empty)");
                        return;
                    }
                    ...
                    if (message.Command == "SetLaunchData")
                    {
                        ...
                        if (data.Length > 200)
                        {
                            App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Data cannot be longer than 200 characters");
                            return;
                        }
                ...
                else if (entry.Contains(GamePlayerJoinLeaveEntry))
                {
                    ...
                    if (match.Groups.Count != 4)
                    {
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, "Failed to assert format for game join/leave log");
                        App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, entry);
                        return;
                    }
                    ...
                    App.Logger.WriteLine(LogLevel.Info,  LOG_IDENT, $"Found player log entry \"{match.Groups[1]} @{match.Groups[2]} ({match.Groups[3]})\"");
                ...
                else if (entry.Contains(GameMessageLogEntry))
                {
                    ...
                    if (match.Groups.Count != 2)
                    {
                        App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Failed to assert format for message log");
                        App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, entry);
                        return;
                    }
                    ...
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Found message log entry \"{match.Groups[1]}\"");

                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, Data.MessageLogs.Count.ToString());
