...
namespace Plexity
{
    ...
    public bool BypassUpdateCheck => UninstallFlag.Active || WatcherFlag.Active;
        ...
        // Define the flags with identifiers
        ...
        public object TestModeFlag { get; internal set; }
        ...
        public LaunchSettings(string[] args)
        {
            ...
            if (Args.Length >= 1)
            {
                ...
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Got Roblox player argument");
            ...
            for (int i = startIdx; i < Args.Length; i++)
            {
                ...
                if (!arg.StartsWith('-'))
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Invalid argument: {arg}");
                    continue;
                }
                ...
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Unknown argument: {identifier}");
                    continue;
                }
                ...
                if (i < Args.Length - 1 && Args[i + 1] is string nextArg && !nextArg.StartsWith('-'))
                {
                    flag.Data = nextArg;
                    i++;
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Identifier '{identifier}' is active with data");
                }
                else
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Identifier '{identifier}' is active");
                }
            }
        ...
        private void ParsePlayer(string? data)
        {
            ...
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Got Roblox launch arguments");
                RobloxLaunchArgs = data;
            }
            else
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "No Roblox launch arguments were provided");
            }
        }
        ...
        private void ParseStudio(string? data)
        {
            ...
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Got Roblox Studio launch arguments");
                RobloxLaunchArgs = data;
            }
            else if (data.StartsWith("roblox-studio-auth:"))
            {
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Got Roblox Studio Auth launch arguments");
            ...
            {
                // likely a local path
                App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, "Got Roblox Studio local place file");
