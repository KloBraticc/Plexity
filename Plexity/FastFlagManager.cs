...
namespace Plexity
{
    public class ClientAppSettings : JsonManager<Dictionary<string, object>>
    {
        public override string ClassName => Path.Combine(Paths.Mods, "ClientSettings", nameof(ClientAppSettings));
        public override string LOG_IDENT_CLASS => ClassName;

        public new string FileLocation => Path.Combine(Paths.Mods, "ClientSettings\\ClientAppSettings.json");
        ...
        public static IReadOnlyDictionary<string, string> PresetFlags = new Dictionary<string, string>
        {
            ...
            //HighResTextures
            { "System.HighResTextures", "DFFlagSkipHighResolutiontextureMipsOnLowMemoryDevices2"},
            ...
            { "Network.LargeReplicatorRead", "FFlagLargeReplicatorRead4"},
             ...
             // Bloom
            { "Lighting.Bloom1", "FIntBloomFrmCutoff"},
            { "Lighting.Bloom2", "FFlagRenderNoLowFrmBloom"},

            // Vignette
            { "WOW.Vignette", "FFlagVignetteEffectEnabled4"},
            ...
            //Diskchecks
            { "System.DiskChecks", "DFFlagAlwaysSkipDiskCache" },

            { "System.SmoothTerrain", "FFlagDebugRenderingSetDeterministic" },

            { "System.BetterShadows", "FFlagRenderInitShadowmaps" },

            //SSAO
            { "Render.SSAOForce", "FFlagDebugSSAOForce" },
            { "Render.SSAOMip", "FIntSSAOMipLevels" },
            { "Render.SSAOLVL", "FIntSSAO" },
            ...
            { "Rendering.UpdateRateMenu", "FIntNetworkStatRefreshRate" },
        ...
        public static IReadOnlyDictionary<SSAOMode, string?> SSAOModes => new Dictionary<SSAOMode, string?>
        {
            { SSAOMode.Default, null },
            { SSAOMode.x1, "1" },
            { SSAOMode.x2, "2" },
            { SSAOMode.x4, "4" },
            { SSAOMode.x5, "5" },
        ...
        public static IReadOnlyDictionary<RefreshRate, string?> RefreshRates => new Dictionary<RefreshRate, string?>
{
    ...
    { RefreshRate.RefreshRate250, "250" },
    { RefreshRate.RefreshRate280, "280" },
    { RefreshRate.RefreshRate300, "300" },
    { RefreshRate.RefreshRate360, "360" },
    { RefreshRate.RefreshRate400, "400" },
    { RefreshRate.RefreshRate480, "480" },
    { RefreshRate.RefreshRate500, "500" },
    { RefreshRate.RefreshRate540, "540" },
    { RefreshRate.RefreshRate600, "600" },
    { RefreshRate.RefreshRate650, "650" },
    { RefreshRate.RefreshRate680, "680" },
};
        ...
        {
            const string LOG_IDENT = "ClientAppSettings::SetValue";

            if (value is null)
            {
                if (Prop.ContainsKey(key))
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Deletion of '{key}' is pending");
            ...
            else
            {
                if (Prop.ContainsKey(key))
                {
                    ...
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Changing of '{key}' from '{Prop[key]}' to '{value}' is pending");
                }
                else
                {
                    App.Logger.WriteLine(LogLevel.Info, LOG_IDENT, $"Setting of '{key}' to '{value}' is pending");
                }
        ...
        public string? GetPreset(string name)
        {
            ...
            {
                App.Logger.WriteLine(LogLevel.Info, "ClientAppSettings::GetPreset", $"Could not find preset {name}");
        ...
        public void DeleteBackup(string Backup)
        {
            ...
            try
            {
                ...
                string BackupPath = Path.Combine(backupsDirectory, Path.GetFileName(Backup));
