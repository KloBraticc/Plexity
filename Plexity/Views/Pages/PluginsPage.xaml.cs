    ...
    public partial class PluginsPage : Page
    {
        ...
        public void LaunchPluginFromDll(string pluginDllPath, string windowClassFullName)
        {
            try
            {
                ...
                {
                    DialogService.ShowMessage($"Plugin DLL not found: {pluginDllPath}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                if (windowType == null)
                {
                    DialogService.ShowMessage($"Window class '{windowClassFullName}' not found in plugin DLL.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                {
                    DialogService.ShowMessage($"Type '{windowClassFullName}' is not a Window.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
                DialogService.ShowMessage($"Failed to load plugin window: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        public void LaunchPluginWindow(string pluginFolder)
        {
            try
            {
                ...
                if (xamlFile == null)
                {
                    DialogService.ShowMessage("No Window XAML file found.", "Error", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                {
                    DialogService.ShowMessage("Plugin code-behind file (.cs) missing.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                if (assembly == null)
                {
                    DialogService.ShowMessage("Compilation failed.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                if (logicType == null)
                {
                    DialogService.ShowMessage("Logic class not found in compiled code.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
            ...
            {
                DialogService.ShowMessage($"Error launching plugin: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void LaunchPlugin_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button btn && btn.DataContext is PluginItem plugin)
            {
                ...
                {
                    DialogService.ShowMessage("Plugin XAML or code-behind file is missing or invalid.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                {
                    DialogService.ShowMessage("Failed to load plugin window:\n" + ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
        ...
        private void RemovePlugin_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.DataContext is PluginItem plugin)
            {
                ...
                {
                    DialogService.ShowMessage($"This Plugin is not Installed.", "Folder Not Found", MessageBoxButton.OK, MessageBoxImage.Information);
                    return; 
                }
                    try
                    {
                    ...
                    {
                        DialogService.ShowMessage($"Failed to delete plugin files:\n{ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
