    ...
    public static class Deployment
    {
        ...
        private static async Task<string?> TestConnection(string url, int delaySeconds, CancellationToken token)
        {
            ...
            try
            {
                ...
                App.Logger.WriteLine(LogLevel.Info, logIdent, "Connecting...");
        ...
        public static async Task<Exception?> InitializeConnectivity()
        {
            ...
            App.Logger.WriteLine(LogLevel.Info, logIdent, "Testing connectivity...");
            ...
            App.Logger.WriteLine(LogLevel.Info, logIdent, $"Using base URL: {BaseUrl}");
            return null;
        }
        ...
        public static async Task<ClientVersion> GetInfo(string? inputChannel = null)
        {
            ...
            App.Logger.WriteLine(LogLevel.Info, logIdent, $"Fetching deploy info for channel {channel}");

            string cacheKey = $"{channel}-{BinaryType}";
            if (ClientVersionCache.TryGetValue(cacheKey, out var cachedVersion))
            {
                App.Logger.WriteLine(LogLevel.Info, logIdent, "Using cached deploy info");
            ...
            {
                App.Logger.WriteLine(LogLevel.Info, logIdent, "Fallback to clientsettings.roblox.com");
                App.Logger.WriteException(logIdent, ex);
            ...
            if (!isDefault && clientVersion?.Version != null)
            {
                var defaultVer = await GetInfo(DefaultChannel);
                if (defaultVer?.Version != null &&
                    (VersionComparison)Utilities.CompareVersions(clientVersion.Version, defaultVer.Version) == VersionComparison.LessThan)
                {
